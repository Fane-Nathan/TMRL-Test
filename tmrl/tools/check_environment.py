...
# local imports
from tmrl.custom.tm.tm_gym_interfaces import TM2020Interface, TM2020InterfaceLidar, TM2020InterfaceLidarCnn
...
def check_env_tm20lidar_cnn():
    window_interface = WindowInterface("Trackmania")
    if cfg.SYSTEM != "Windows":
        window_interface.move_and_resize()  # needed on Linux
    lidar = Lidar(window_interface.screenshot())
    env_config = DEFAULT_CONFIG_DICT.copy()
    env_config["interface"] = TM2020InterfaceLidarCnn
    env_config["wait_on_done"] = True
    env_config["interface_kwargs"] = {"img_hist_len": 1, "gamepad": False, "grayscale": cfg.GRAYSCALE, "resize_to": (cfg.IMG_WIDTH, cfg.IMG_HEIGHT)}
    env = gymnasium.make(cfg.RTGYM_VERSION, config=env_config)
    o, i = env.reset()
    show_imgs(o[3])
