...
def get_local_buffer_sample_lidar_cnn(prev_act, obs, rew, terminated, truncated, info):
    """
    Sample compressor for MemoryTMLidarCnn
    """
    prev_act_mod = prev_act
    # obs: [speed, gear, rpm, imgs, lidar]
    # Store: speed, gear, rpm, lidar, last_img_uint8
    obs_mod = (obs[0], obs[1], obs[2], obs[4], (obs[3][-1] * 256.0).astype(np.uint8))
    rew_mod = rew
    terminated_mod = terminated
    truncated_mod = truncated
    info_mod = info
...
class MemoryTMLidarCnn(MemoryTM):
    def get_transition(self, item):
        """
        CAUTION: item is the first index of the 4 images in the images history of the OLD observation
        CAUTION: in the buffer, a sample is (act, obs(act)) and NOT (obs, act(obs))
        ...
        # Flatten lidar if necessary? No, Memory returns (speed, gear, rpm, imgs, lidars, actions)
        # obs: (speed, gear, rpm, imgs, lidar)
        # stored: speed, gear, rpm, lidar, img
        lidar_last = self.data[3][idx_last]
        lidar_now = self.data[3][idx_now]

        last_obs = (self.data[2][idx_last], self.data[7][idx_last], self.data[8][idx_last], imgs_last_obs, lidar_last, *last_act_buf)
        new_act = self.data[1][idx_now]
        rew = np.float32(self.data[5][idx_now])
        new_obs = (self.data[2][idx_now], self.data[7][idx_now], self.data[8][idx_now], imgs_new_obs, lidar_now, *new_act_buf)
        terminated = self.data[9][idx_now]
        truncated = self.data[10][idx_now]
        info = self.data[6][idx_now]
        return last_obs, new_act, rew, new_obs, terminated, truncated, info
    ...
    def append_buffer(self, buffer):
        ...
        obs: [speed, gear, rpm, lidar, last_img_uint8] (from get_local_buffer_sample_lidar_cnn)
        """
        ...
        d1 = [b[0] for b in buffer.memory]  # actions
        d2 = [b[1][0] for b in buffer.memory]  # speeds
        d3 = [b[1][3] for b in buffer.memory]  # lidar
        d4 = [b[3] or b[4] for b in buffer.memory]  # eoes
        d5 = [b[2] for b in buffer.memory]  # rewards
        d6 = [b[5] for b in buffer.memory]  # infos
        d7 = [b[1][1] for b in buffer.memory]  # gears
        ...
        d9 = [b[3] for b in buffer.memory]  # terminated
        d10 = [b[4] for b in buffer.memory]  # truncated
        d11 = [b[1][4] for b in buffer.memory] # images
